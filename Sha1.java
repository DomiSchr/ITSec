	...
	public static int[] sha1(BitBlock message) {
		...
		while (globalWordCounter < message.wordCount) {
			...
			for (roundCounter = 0; roundCounter < 16; roundCounter++) {
				z = e1 + ((b1 & c1) | ((~b1) & d1)) + (((a1 & rot5HighMask) >>> 27) + (a1 << 5)) + msf[roundCounter]
						+ 0x5A827999;
				e1 = d1;
				d1 = c1;
				c1 = ((b1 & 3) << 30) + (b1 >>> 2);
			...
			for (; roundCounter < 20; roundCounter++) {
				z = msf[(roundCounter - 16) % 16] ^ msf[(roundCounter - 14) % 16] ^ msf[(roundCounter - 8) % 16]
						^ msf[(roundCounter - 3) % 16];
				msf[roundCounter % 16] = (z << 1) | ((z & 0x80000000) >>> 31);

				z = e1 + ((b1 & c1) | ((~b1) & d1)) + (((a1 & rot5HighMask) >>> 27) + (a1 << 5))
						+ msf[roundCounter % 16] + 0x5A827999;
				e1 = d1;
				d1 = c1;
				c1 = ((b1 & 3) << 30) + (b1 >>> 2);
				b1 = a1;
			...
			for (; roundCounter < 40; roundCounter++) {
				...
				z = e1 + (b1 ^ c1 ^ d1) + (((a1 & rot5HighMask) >>> 27) + (a1 << 5)) + msf[roundCounter % 16]
						+ 0x6ED9EBA1;
				e1 = d1;
				d1 = c1;
				c1 = ((b1 & 3) << 30) + (b1 >>> 2);
			...
			for (; roundCounter < 60; roundCounter++) {
				...
				z = e1 + ((b1 & c1) | (b1 & d1) | (c1 & d1)) + (((a1 & rot5HighMask) >>> 27) + (a1 << 5))
						+ msf[roundCounter % 16] + 0x8F1BBCDC;
				e1 = d1;
				d1 = c1;
				c1 = ((b1 & 3) << 30) + (b1 >>> 2);
				b1 = a1;
